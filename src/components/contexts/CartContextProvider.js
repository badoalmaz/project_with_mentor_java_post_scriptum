...
const CartContextProvider = ({ children }) => {
    ...
    function deletProductInCart(id) {
        ...
        cart.totalPrice = calcTotalPrice(cart.products);
        localStorage.setItem("catr", JSON.stringify(cart));
        getCart();
        dispatch({
            type: CART.GET_CART,
            payload: cart,
    ...
    const changeProductCount = (count, id) => {
        ...
        cart.products = cart.products.map((product) => {
            if (product.item.id === id) {
                product.count = count;
                product.subPrice = calcSubPrice(cart.products);
            }
            return product;
        });
        cart.totalPrice = calcTotalPrice(cart.price);
        dispatch({
            type: CART.GET_CART,
            payload: cart,
        });
    ...
    const values = { getCart, addProductToCart, changeProductCount, deletProductInCart, checkProductInCart, cart: state.cart, };
